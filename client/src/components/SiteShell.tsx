...
import { Clock3, Facebook, Mail, MapPin, Menu, MessageCircle, Phone, Settings2, X } from "lucide-react";
    ...
    <>
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-[#f7f6f1]/95 backdrop-blur-xl">
        <div className="mx-auto flex h-[84px] max-w-7xl items-center justify-between px-4 sm:px-8">
          <Link href="/" className="flex items-center" aria-label="ETS Pro-Informatique, accueil"><img src={officialLogo} alt="Logo officiel Informatique" className="h-16 w-40 object-contain object-left" /></Link>
          ...
          <div className="hidden items-center gap-2 sm:flex"><Link href="/parametres" className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-slate-600 transition hover:-translate-y-0.5 hover:border-cyan-700 hover:text-cyan-800" aria-label="Ouvrir les paramètres"><Settings2 className="h-4 w-4" /></Link><Link href="/contact" className="items-center gap-2 rounded-full bg-[#68B62A] px-5 py-3 text-sm font-extrabold text-white shadow-[0_10px_22px_rgba(104,182,42,0.25)] transition hover:-translate-y-0.5 hover:bg-[#579c20] active:scale-[0.97] sm:flex"><MessageCircle className="h-4 w-4" /> Demander un devis</Link></div>
          <button onClick={() => setMenuOpen(!menuOpen)} className="menu-toggle grid h-11 w-11 place-items-center rounded-full border border-slate-200 bg-white text-slate-900 xl:hidden" aria-label={menuOpen ? "Fermer le menu" : "Ouvrir le menu"}>{menuOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button>
        </div>
        {menuOpen && <div className="mobile-menu-panel border-t border-slate-200 bg-[#f7f6f1] px-6 py-5 xl:hidden"><nav className="flex flex-col gap-4 text-base font-bold text-slate-800">{routes.map((route) => <Link key={route.href} href={route.href} onClick={closeMenu} className={location === route.href ? "text-cyan-800" : ""}>{route.label}</Link>)}<Link href="/parametres" onClick={closeMenu} className="flex items-center gap-2 text-cyan-800"><Settings2 className="h-4 w-4" /> Paramètres</Link><a href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noreferrer" className="mt-1 flex w-fit items-center gap-2 rounded-full bg-[#68B62A] px-5 py-3 text-sm text-white"><MessageCircle className="h-4 w-4" /> WhatsApp</a></nav></div>}
      </header>
    </>
  );
}
